...
function GalleryScene({
	...
	textures = [],
}: Omit<InfiniteGalleryProps, 'className' | 'style'> & { textures: THREE.Texture[] }) {
		...
		normalizedImages.forEach((img, index) => {
			textureLoader.load(
				...
				(texture) => {
					...
					loaded++;
					setLoadingProgress((loaded / normalizedImages.length) * 100);
					if (loaded === normalizedImages.length) {
